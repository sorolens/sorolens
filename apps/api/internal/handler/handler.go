...
type Handler struct {
	...
	// BuildInfo carries the ldflags-injected build metadata reported by
	// GET /api/v1/version. An unset field falls back to its default, so a
	// handler that never sets it still answers with all four fields.
	BuildInfo BuildInfo
}
