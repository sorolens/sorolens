...
func New(h *handler.Handler) http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		// Build metadata. Read-only and not in the scope table, so it stays
		...
		// Watchdog alert subscriptions. Listing is watchdog-scoped; anything
		// that mints a subscription or touches its signing secret is
		// admin-gated, because the secret is signing key material.
		get("/watchdog/subscriptions", h.ListSubscriptions)
		r.With(scope, admin).Post("/watchdog/subscriptions", h.CreateSubscription)
		r.With(scope, admin).Delete("/watchdog/subscriptions/{id}", h.DeleteSubscription)
