...
// Build metadata injected with -ldflags at build time; see the "Build API
// binary with build-info ldflags" step in .github/workflows/ci.yml.
var (
	// Commit is the git SHA the binary was built from.
	Commit = handler.DefaultCommit
	// BuildDate is the RFC 3339 UTC timestamp of the build.
	BuildDate = handler.DefaultBuildDate
)
