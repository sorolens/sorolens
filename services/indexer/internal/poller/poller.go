...
func (p *Poller) processContract(ctx context.Context, contract Contract) error {
	...
	runStart := time.Now()
	events, invocations, callEdges, err := p.fetchWindow(ctx, rpc, contractID, network, startLedger, endLedger)
	if err != nil {
		return err
	}

	newState := SyncState{ContractID: contractID, LastLedger: endLedger}
	if err := p.store.BatchInsertWithCursor(ctx, network, endLedger, events, invocations, callEdges, newState); err != nil {
...
//
// For each transaction it also reconstructs the cross-contract call tree from
// the returned diagnostic events (fn_call / fn_return / core_metrics) and
// returns one edge per non-root frame. A transaction with no diagnostic events
// (the public-RPC default) contributes no edges and is not an error.
func (p *Poller) fetchWindow(ctx context.Context, rpc RPCClient, contractID, network string, startLedger, endLedger uint32) ([]Event, []Invocation, []CallEdge, error) {
	filters := []EventFilter{{
		Type:        "contract",
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("get events [%d,%d]: %w", startLedger, endLedger, err)
	}
	...
	// Fetch one transaction record per unique tx hash, and materialise each
	// transaction's cross-contract call graph from its diagnostic events.
	...
	for txHash := range seenTx {
		...
		invocations = append(invocations, Invocation{
			...
			ResourceFeeCharged: tx.ResourceFee,
			ApplicationOrder:   tx.ApplicationOrder,
		})

		edges := callgraph.Parse(txHash, tx.DiagnosticEventsXDR)
		if len(edges) == 0 {
			continue
		}
		// Attribute the root invocation's resource fee across the top-level
		// frames so the dashboard can show where the fee went.
		edges = callgraph.DistributeFee(tx.ResourceFee, edges)
		for _, e := range edges {
			callEdges = append(callEdges, CallEdge{
				TxHash:           e.TxHash,
				ParentSpanID:     e.ParentSpanID,
				ChildSpanID:      e.ChildSpanID,
				CalleeContractID: e.CalleeContractID,
	...
	return events, invocations, callEdges, nil
}
