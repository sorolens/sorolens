    ...
    try {
      ...
      try {
        if (interaction.deferred || interaction.replied) {
          ...
          // MessageFlags.Ephemeral === 1 << 6 (64). Using the numeric
          // literal avoids importing MessageFlags for a single flag.
          await interaction.reply({ content: `Something went wrong: ${msg}`, flags: 64 });
        }
      } catch (reportErr) {
        // The interaction may have been acknowledged by another code
        // path (or expired). Swallow so we do not throw an unhandled
        // rejection from the top-level handler.
